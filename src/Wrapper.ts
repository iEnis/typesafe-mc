...
const exists = (module: string) => existsSync(paths.exec(`/node_modules/${module}/index.d.ts`));

export default class Wrapper {
    protected constructor() {}
    public static activeSpinner?: { spinner: Spinner; message: string };

    public static moduleCheck() {
        const modules: string[] = [];

        // if (exists("@minecraft/server")) modules.push("@minecraft/server");
        if (exists("@minecraft/server-ui")) modules.push("@minecraft/server-ui");
    ...
    public static exit(message = "Canceled!") {
